...
function renderRoutePointLabels(): string {
  return siteData.route.points
    .map((point, index) => {
      ...
      const position = routeGeometry.getPointAtLength(routeGeometry.getTotalLength() * point.progress)

      return `
        <g class="route-marker" data-progress="${point.progress}" transform="translate(${position.x.toFixed(3)} ${position.y.toFixed(3)})" aria-hidden="true">
      ...
      <div class="route-scrolly" data-route-scrolly>
        <div class="route-visual" aria-label="Schemat badanego odcinka">
          ...
          <svg class="route-svg" viewBox="0 0 520 900" role="img" aria-labelledby="route-svg-title route-svg-desc">
            ...
            <path id="route-line" class="route-road" d="${routePath}" pathLength="1" />
            <path class="route-progress" d="${routePath}" pathLength="1" />
            <path class="route-centerline" d="${routePath}" pathLength="1" />
